class Solution {
    public long minimumCost(int[] nums) {
        Arrays.sort(nums);
        int n = nums.length;
        int m = n % 2 == 1 ? nums[n / 2] : (nums[(n - 1) / 2] + nums[n / 2]) / 2;
        long a = getNextGreaterPolindrome(m);
        long b = getNextSmallerPolindrome(m);
        return Math.min(isPalindrome(m) ? getCost(nums, m) : Long.MAX_VALUE, Math.min(getCost(nums, a), getCost(nums, b)));
    }
    
    long getCost(int[] nums, long p) {
        long ans = 0;
        for (int num : nums)
            ans += Math.abs((long)num - p);
        return ans;
    }
